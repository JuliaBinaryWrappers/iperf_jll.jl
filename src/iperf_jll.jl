...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import iperf_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("c3111fcb-6a66-54cb-a6df-fd652d9c82a2"))
platforms = [Pkg.Artifacts.unpack_platform(e, "iperf", artifacts_toml) for e in artifacts["iperf"]]
...
if best_platform === nothing
    @debug("Unable to load iperf; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module iperf_jll
